"""Recursive self-correction engine — Optuna-backed parameter evolution.

Connects to the Railway PostgreSQL database as the Optuna storage
backend for study persistence and multi-worker coordination.  Fetches
the last 1,000 trades from ``ml_training_data``, simulates indicator
logic under candidate parameters, and maximises the annualised Sortino
ratio over 50 trials.

After optimisation the engine generates a JSON "Parameter Mutation"
payload for ClaudeCTO semantic verification before values are
hot-swapped into Redis.

Usage::

    engine = RecursiveEvolutionEngine()
    mutation = engine.run(n_trials=50)
    # mutation.payload  -> JSON dict ready for ClaudeCTO verification
    # mutation.best_sortino -> float
"""
...
_STUDY_NAME: str = "oniquant_evolution"
_TRADE_LOOKBACK: int = int(os.getenv("OPTIMIZER_TRADE_LOOKBACK", "1000"))
...
logger = logging.getLogger("oniquant.recursive_engine")

# Suppress Optuna's default INFO chatter during trial runs.
optuna.logging.set_verbosity(optuna.logging.WARNING)

# ---------------------------------------------------------------------------
# Lightweight ORM model (sync, decoupled from async Base)
# ---------------------------------------------------------------------------
...
class _MLTrainingData(_Base):
    """Read-only projection of ``ml_training_data`` for the engine."""
...
class ParameterMutation:
    """Immutable payload produced after an evolution cycle.

    ``payload`` is the JSON-serialisable dict intended for ClaudeCTO
    semantic verification before hot-swapping into Redis.
    """
...
def _compute_sortino(returns: np.ndarray) -> float:
    r"""Annualised Sortino ratio (MAR = 0).

    .. math::

        S = \frac{\text{mean return}}{\text{downside std}} \times \sqrt{252}
...
def _objective(trial: optuna.Trial, trades: list[dict[str, Any]]) -> float:
    ...
    Fetches the candidate search space dynamically via define-by-run,
    filters the historical trade population, and returns the annualised
    Sortino ratio as the optimisation target.
    """
...
# ---------------------------------------------------------------------------
# Parameter Mutation payload builder
# ---------------------------------------------------------------------------
...
def _build_mutation_payload(
    ...
    """Build the JSON payload for ClaudeCTO semantic verification.

    The payload groups parameters by desk so ClaudeCTO can reason
    about per-desk changes relative to historical performance.
    """
    best = study.best_trial
    params = dict(best.params)

    # Group parameters by desk prefix.
    desk_params: dict[str, dict[str, Any]] = {}
    cross_desk: dict[str, Any] = {}

    for key, value in sorted(params.items()):
        if key.startswith("d") and "_" in key:
            prefix = key.split("_")[0]  # e.g. "d1", "d2"
            desk_params.setdefault(prefix, {})[key] = value
        else:
            cross_desk[key] = value

    return {
        "mutation_type": "parameter_evolution",
        "study_name": study.study_name,
        "generation": best.number,
        "sortino_ratio": round(best.value, 6),
        "trades_evaluated": trades_count,
        "accepted_trades": best.user_attrs.get("n_accepted_trades", 0),
        "mean_return": round(best.user_attrs.get("mean_return", 0.0), 8),
        "win_rate": round(best.user_attrs.get("win_rate", 0.0), 4),
        "desk_parameters": desk_params,
        "cross_desk_parameters": cross_desk,
        "top_param_importances": {
...
class RecursiveEvolutionEngine:
    """Autonomous self-correction engine backed by Optuna + PostgreSQL.

    Connects to Railway PostgreSQL as the storage backend for study
    persistence and multi-worker coordination.  Defines an objective
    function that simulates current indicator logic over historical
    trades, optimises for the annualised Sortino ratio, and produces
    a Parameter Mutation payload for ClaudeCTO verification.

    Parameters
    ----------
    postgres_url : str | None
        Synchronous SQLAlchemy connection URL.  Falls back to
        ``POSTGRES_URL`` env var (``+asyncpg`` stripped automatically).
    trade_lookback : int
    ...
    def __init__(
        ...
        trade_lookback: int = _TRADE_LOOKBACK,
        sampler: optuna.samplers.BaseSampler | None = None,
        pruner: optuna.pruners.BasePruner | None = None,
    ) -> None:
        self._pg_url: str = (
            postgres_url or SYNC_POSTGRES_URL
        ).replace("+asyncpg", "")
    ...
    def _get_or_create_study(self) -> optuna.Study:
        ...
        Uses the exact pattern specified::

            optuna.create_study(
                study_name="oniquant_evolution",
                storage=os.getenv("POSTGRES_URL"),
                load_if_exists=True,
                direction="maximize",
            )
        """
    ...
    def run(
        ...
        """Execute an evolution cycle and produce a Parameter Mutation.
        ...
        -------
        ParameterMutation
            Frozen result with ``payload`` dict ready for ClaudeCTO
            semantic verification before Redis hot-swap.
        """
        self._trades = self._load_trades()
        if not self._trades:
            logger.warning(
                "No trades in ml_training_data — returning empty mutation"
            )
        ...
        logger.info(
            "RecursiveEvolutionEngine starting: study=%s, trades=%d, trials=%d",
            _STUDY_NAME,
            len(self._trades),
            n_trials,
        )
        ...
        if len(completed) >= 2:
            try:
                ...
                logger.debug(
                    "Could not compute param importances", exc_info=True,
                )

        # Build the Parameter Mutation payload for ClaudeCTO.
        ...
        logger.info(
            "Evolution complete — best Sortino=%.4f (trial #%d)\n"
            "  params=%s\n"
            "  mutation payload keys=%s",
            mutation.best_sortino,
            mutation.best_trial_number,
            mutation.best_params,
            list(payload.keys()),
        )
    ...
    def get_desk_params(self, desk_id: int) -> dict[str, Any]:
        ...
        Strips the desk prefix (e.g. ``d2_``) and returns a clean dict
        suitable for passing to the desk's strategy constructor.
        """
